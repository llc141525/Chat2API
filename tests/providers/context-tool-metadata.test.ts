...
test('summary keeps generated summary while restoring related tool exchange pairs', async () => {
  ...
  assert.ok(result.messages.some((message) => message.content === '[Conversation Summary]\ntrimmed summary'))
  assert.ok(result.messages.some((message) => message.role === 'assistant' && message.tool_calls?.[0]?.id === 'call_summary'))
  assert.ok(result.messages.some((message) => message.role === 'tool' && message.tool_call_id === 'call_summary'))
})
