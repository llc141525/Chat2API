...
export class MimoAdapter {
  ...
  async chatCompletionWithAssembly(
    ...
    // Enhanced system with summary and tool contract
    let enhancedSystem = systemText
    ...
    // Build conversation entries (same logic as buildMimoQuery, skipping system since handled via enhancedSystem)
    const toolProfile = getProviderToolProfile('mimo')
    const entries: Array<{ role: string; content: string }> = []

    for (const message of assembly.messages) {
      if (message.role === 'system') {
        continue // Skip system messages - handled above via enhancedSystem
      }
    ...
    if (entries.length === 1 && entries[0].role === 'User') {
      ...
      query = entries.map((entry) => `${entry.role}: ${entry.content}`).join('\n\n')
    }

    // Prepend enhanced system text to query
    if (enhancedSystem) {
      query = `${enhancedSystem}\n\n${query}`
    }

    // === End of assembly-based query building ===
