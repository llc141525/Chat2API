...
10. Use the `read` tool to read `.agent-probe/long-environment.json` and verify that the recorded `cwd` is the current tool workspace.
11. Output only the exact final marker assembled from `LONG`, `CONVERSATION`, `PROBE`, `DONE` with underscores.

The valid action order is `skill -> read -> bash -> read -> bash -> bash -> bash -> bash -> bash -> read -> read -> final text`.
